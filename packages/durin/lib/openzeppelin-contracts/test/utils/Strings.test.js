...
describe("Strings", () => {
	...
	describe("toHexString", () => {
		it("converts 0", async function () {
			expect(await this.mock.getFunction("$toHexString(uint256)")(0n)).to.equal(
				"0x00",
			);
		});
		...
		it("converts a positive number", async function () {
			expect(
				await this.mock.getFunction("$toHexString(uint256)")(0x4132n),
			).to.equal("0x4132");
		});

		it("converts MAX_UINT256", async function () {
			expect(
				await this.mock.getFunction("$toHexString(uint256)")(ethers.MaxUint256),
	...
	describe("addresses", () => {
		...
		describe("toChecksumHexString", () => {
			for (const addr of addresses) {
				it(`converts ${addr}`, async function () {
					expect(
						await this.mock.getFunction("$toChecksumHexString(address)")(addr),
					).to.equal(ethers.getAddress(addr.toLowerCase()));
				});
			}
		});
	});
