		...
		describe("with tampered values", () => {
			for (const [key, value] of Object.entries(tamperedValues)) {
				it(`returns false with tampered ${key}`, async function () {
					const request = await this.forgeRequest();
					request[key] =
						typeof value === "function" ? value(request[key]) : value;
		...
		describe("with tampered request", () => {
			for (const [key, value] of Object.entries(tamperedValues)) {
				it(`reverts with tampered ${key}`, async function () {
					...
					const promise = this.forwarder.execute(request, {
			...
			describe("when the refund receiver is the zero address", () => {
				...
				for (const [key, value] of Object.entries(tamperedValues)) {
					it(`reverts with at least one tampered request ${key}`, async function () {
						this.requests[idx][key] =
							typeof value === "function"
								? value(this.requests[idx][key])
								: value;

						const promise = this.forwarder.executeBatch(
							this.requests,
							this.refundReceiver,
							{ value: this.value },
						);
