...
export async function GET(
  ...
  try {
    ...
    const videoKey = decodeURIComponent(resolvedParams.videoKey);
    ...
    if (range) {
      ...
      end = parts[1] ? Number.parseInt(parts[1], 10) : contentLength - 1;
    }
    ...
    return new NextResponse(bytes as any, { status, headers });
  } catch (error) {
