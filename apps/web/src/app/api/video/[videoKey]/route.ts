...
export async function GET(request: NextRequest, { params }: { params: { videoKey: string } }) {
  ...
  try {
    ...
    const { ContentLength, ContentType } = await s3Client.send(headObjectCommand);

    if (range) {
      const bytes = range.replace(/bytes=/, '').split('-');
      const start = parseInt(bytes[0], 10);
      const end = bytes[1] ? parseInt(bytes[1], 10) : ContentLength! - 1;
